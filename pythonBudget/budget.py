...
Objective: Uses recursion to find the nested values of:
           amount, date, and account.

Accepts following parameters:
    trans: a transaction element
    key: str key to the namespace dictionary
    info: value of key from info dictionary.
    ...
    for el in trans.findall('trn:' + key, ns):
        ...
        if type(info[0]) is dict:
            ...
            for item in info:
                ...
                # there are 2 splits, one where the value is negative
                # and one that is positive
                # the negative takes the money category in savings
                # the positive marks how much was spent from category
...
Objective: Loop through each transaction and
           get info specified in info dict.

           If all information is found,
           then it is added to the list of transactions.
...
def get_info(root, accounts):
    ...
    # keys are the info thats needed and values are
    # custom prefix with specific tag where info is stored
    info = {
    ...
    for trans in root.iter('{http://www.gnucash.org/XML/gnc}transaction'):
        ...
        if 'Paycheck' not in val.text:
            ...
            for key in info.keys():
                if key != 'description':
                    found = find_nested_values(trans,
                                               key,
                                               info[key],
                                               found,
                                               accounts)
...
def get_accounts(root):
    ...
    for accnt in root.iter('{http://www.gnucash.org/XML/gnc}account'):
        ...
        if accnt.find('act:type', ns).text == 'EXPENSE':
...
def main():
    parser = argparse.ArgumentParser(description='The purpose of this script: '
                                     'Print transactions to a csv file that '
                                     'can be run in OpenLibre office'
                                     'and Excel.'
                                     'Script will print all transactions'
                                     ' for each year + month by default')
    parser.add_argument('-y', '--year', type=int,
                        help='(Num) Limit transactions for year. Ex -y 2016',
                        required=False)
    parser.add_argument('-m', '--month', type=int,
                        help='(Num) Limit transactions per month.'
                        ' Add year to limit further. Ex. -m 01',
                        required=False)
    parser.add_argument('-a', '--account', type=str,
                        help='(String) Limit transactions for account.'
                        ' Use Quotes for words with spaces', required=False)
